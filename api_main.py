...
def hello_world():
    ...
    if diabetes == '1':
        ...
        test = [[float(pregnancies), float(glucose), float(bloodpressure), float(skinthickness), float(insulin), float(bmi), float(diabetespedigreefunction), float(age)]]
        sc = StandardScaler()
    ...
    if cvd == '1':
        ...
        def get_input(age, gender, height, weight, ap_hi, ap_lo, chol, gluc, smoke, alco, active):
            ...
            return [age, gender, height, weight, ap_hi, ap_lo, smoke, alco, active, chol1, chol2, chol3, gluc1, gluc2, gluc3]
